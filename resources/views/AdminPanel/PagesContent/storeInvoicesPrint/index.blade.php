                ...
                <div class="col-sm-12">
                    <div class="row mb-1 mt-2">
                        <div class="col-sm-5 row" style="border: 2px solid #f00;border-radius: 10px; padding: 5px">
                            ...
                            <div class="col-sm-5">
                              1  <a class="btn btn-outline-primary" href="{{route('updateOracleInvoices')}}">Update
                                    Oracle
                                    Invoices</a>
                            </div>
                            <div class="col-sm-5">
                        ...
                        <div class="col-sm-6 row">
                            <h5 class="col-md-12">Total Invoices Count : {{$storeInvoicesPrint->total()}}</h5>
                        </div>
        ...
        <div class="card-body" style="overflow-x:scroll">
            @if(count($storeInvoicesPrint) > 0)
                <table id="orderHeadersTable" style="width: 100%" class="table table-bordered table-striped">
                    ...
                    <tbody>
                    @foreach($storeInvoicesPrint as $row)
                        <tr>
                            <td>{{$row->Order_ID}}</td>
                            ...
                            <td>{{date('d-m-Y h:m a', strtotime($row->created_at))}}</td>
                              <td>
                                <a class="btn btn-success" href="{{url('/admin/storeInvoicesPrint/printInvoice/'.$row->Order_ID)}}" target="_blank">Print
                                    Invoice</a>
                            </td>
                        </tr>
                    @endforeach
                ...
                <div class="pagination justify-content-center mt-2">
                    ...
                    @if (isset($storeInvoicesPrint) && $storeInvoicesPrint->lastPage() > 1)
                        <ul class="pagination align-items-center">
                        @php
                            $interval = isset($interval) ? abs(intval($interval)) : 3 ;
                            $from = $storeInvoicesPrint->currentPage() - $interval;
                            if($from < 1){
                              $from = 1;
                            }

                            $to = $storeInvoicesPrint->currentPage() + $interval;
                            if($to > $storeInvoicesPrint->lastPage()){
                              $to = $storeInvoicesPrint->lastPage();
                            }
                        @endphp
                        <!-- first/previous -->
                            @if($storeInvoicesPrint->currentPage() > 1)
                                <li class="page-item">
                                    <a href="{{ $storeInvoicesPrint->url(1)."&type=".app('request')->input('type')}}" aria-label="First" class="page-link">
                                        <span aria-hidden="true">&laquo;</span>
                                    </a>
                                </li>
                                <li class="page-item">
                                    <a href="{{ $storeInvoicesPrint->url($storeInvoicesPrint->currentPage() - 1)."&type=".app('request')->input('type') }}" aria-label="Previous" class="page-link">
                        ...
                        <!-- links -->
                            @for($i = $from; $i <= $to; $i++)
                                @php
                                    $isCurrentPage = $storeInvoicesPrint->currentPage() == $i;
                                @endphp
                                <li class="page-item {{ $isCurrentPage ? 'active' : '' }}" style="padding: 5px">
                                    <a class="page-link" href="{{ !$isCurrentPage ? $storeInvoicesPrint->url($i)."&type=".app('request')->input('type') : '' }}">
                                        {{ $i }}
                                    </a>
                                </li>
                            @endfor
                        <!-- next/last -->
                            @if($storeInvoicesPrint->currentPage() < $storeInvoicesPrint->lastPage())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $storeInvoicesPrint->url($storeInvoicesPrint->currentPage() + 1)."&type=".app('request')->input('type') }}" aria-label="Next">
                                        <span aria-hidden="true">&rsaquo;</span>
                                    </a>
                                </li>
                                <li class="page-item">
                                    <a class="page-link" href="{{ $storeInvoicesPrint->url($storeInvoicesPrint->lastpage())."&type=".app('request')->input('type') }}" aria-label="Last">
